...
TARGET_SEARCH_STATE_TOPIC = "/target_search/state"
TARGET_SEARCH_GOAL_TOPIC = "/target_search/goal"
NAV2_STATUS_TOPICS = {
...
def main() -> None:
    ...
    target_search_state_samples: list[tuple[int, str]] = []
    ...
    target_search_goal_samples: list[tuple[int, float, float]] = []
    ...
    for topic_id, timestamp, data in conn.execute(
        ...
        elif topic_name == TARGET_SEARCH_STATE_TOPIC:
            msg = deserialize_message(data, state_type)
            target_search_state_samples.append((timestamp, msg.data))
        ...
        elif topic_name == TARGET_SEARCH_GOAL_TOPIC:
            msg = deserialize_message(data, goal_type)
            target_search_goal_samples.append(
                (timestamp, msg.pose.position.x, msg.pose.position.y)
            )
    ...
    target_search_state_counts = count_states(target_search_state_samples)
    target_search_state_durations_s = estimate_state_durations_s(
        target_search_state_samples,
        bag_end_t,
    )
    ...
    target_search_success = any(
        state == "TARGET_REACHED" for _, state in target_search_state_samples
    )
    ...
    print(f"target_search_state_samples: {len(target_search_state_samples)}")
    ...
    print(f"target_search_goal_samples: {len(target_search_goal_samples)}")
    ...
    if not target_samples:
        ...
        print_xy_summary("target_search_goal", target_search_goal_samples)
        ...
        print_state_summary(
            ...
            target_search_state_durations_s,
        )
        ...
        print(f"target_search_success: {target_search_success}")
        print(
            "success: "
            f"{waypoint_success or mission_success or nav2_success or nav2_odom_success or explorer_success or target_search_success}"
        )
        return
    ...
    print(
        "success: "
        f"{success or waypoint_success or mission_success or nav2_success or nav2_odom_success or explorer_success or target_search_success}"
    )
